...
const books = ["Java", "Spm", "Pom", "Project", "DataWarehouse"];
let books1 = books.map((val) => val.toUpperCase());
console.log(books1);
//2.crate new array that containt only string with length greater then 5:
let books2 = books.filter((val) => val.length > 5);
console.log(books2);
//3. crate new array that containt only string which string name Start with "P":
let books3 = books.filter((value) => value[0] === "P");
console.log(books3);
